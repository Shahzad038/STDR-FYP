package com.student.smartETailor.constants;

public class Const {
    public static final String STATUS_ACTIVE = "active";
    public static final String STATUS_BLOCKED = "blocked";

    public static final String USER_TYPE_TAILOR = "tailor";
    public static final String USER_TYPE_CUSTOMER = "customer";
    public static final String USER_TYPE_RIDER = "rider";

    public static final String USER_ACCESS_USER = "user";
    public static final String USER_ACCESS_ADMIN = "admin";

    public static final String DB_USERS = "Users";
    public static final String DB_USERS_MEASUREMENTS = "Measurements";

    public static final String DB_ORDERS = "Orders";
    public static final String DB_DESIGNS = "Designs";


    public static final String ORDER_STATUS_PENDING = "Pending";
    public static final String ORDER_STATUS_ACCEPTED = "Accepted";
    public static final String ORDER_STATUS_STARTED = "Started";
    public static final String ORDER_STATUS_SUIT_COMPLETED = "Suit Completed";
    public static final String ORDER_STATUS_SHIPPED = "Shipped";
    public static final String ORDER_STATUS_COMPLETED = "Completed";

}
